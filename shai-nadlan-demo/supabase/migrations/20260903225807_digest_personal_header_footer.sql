drop function if exists public.digest_shell(text, text, int);

-- Same five sections, verified by eye and unchanged. What changes is the voice
-- around them: it greets him the way the app does, tells him in a sentence how
-- much is waiting, and signs off. A weekly mail that opens "מה מחכה לך" reads
-- like a tab title; one that opens "בוקר טוב, שי" reads like it is for him.
...
declare
  ...
  v_close text;
begin
  select coalesce(greeting_name, 'שי') into v_name from public.digest_settings where id;
  v_name := coalesce(v_name, 'שי');
  ...
  v_open := case
    when v_n = 0 then 'שבוע שקט. לא צריך לעשות כלום.'
    when v_n = 1 then 'יש דבר אחד שמחכה לך.'
    when v_n = 2 then 'יש שני דברים שמחכים לך.'
    when v_n = 3 then 'יש שלושה דברים שמחכים לך.'
    when v_n = 4 then 'יש ארבעה דברים שמחכים לך.'
    else 'יש ' || v_n || ' דברים שמחכים לך.'
  end;
  v_close := case when v_n = 0 then 'נתראה בשבוע הבא.'
                  else 'הכול מחכה לך במערכת — נתראה בשבוע הבא.' end;
  ...
  subject := case when v_n = 0
                  then 'שבוע שקט, ' || v_name || ' · ' || to_char(v_today,'DD.MM')
                  else v_name || ', ' || lower('') || v_open || ' · ' || to_char(v_today,'DD.MM') end;
  ...
  html :=
    ...
    ||   '<div style="font-size:28px;font-weight:700;color:#000;letter-spacing:-.4px;padding-top:2px;">'
    ||     public.digest_greeting() || ', ' || v_name || '</div>'
    ||   '<div style="font-size:16px;color:#3C3C4399;padding-top:5px;line-height:1.5;">' || v_open || '</div>'
    || '</td></tr>'
    || v_body
    ...
    || '</td></tr>'
    || '<tr><td align="center" style="padding:18px 6px 4px 6px;font-size:14px;color:#3C3C4399;">' || v_close || '</td></tr>'
    || '</table></div>';
